...
HalResult LinuxHalInput::init() {
    try {
        std::cout << "Initializing Linux touch input" << std::endl;
        ...
        for (int i = 0; touch_devices[i] != nullptr; i++) {
            ...
            if (touch_fd_ != -1) {
                std::cout << "Opened touch device: " << touch_devices[i] << std::endl;
                break;
            }
        }
        
        if (touch_fd_ == -1) {
            std::cout << "No touch device found, using mouse simulation" << std::endl;
        ...
        if (!input_device_) {
            std::cerr << "Failed to create LVGL input device" << std::endl;
        ...
        std::cerr << "Exception during Linux input init" << std::endl;
        return HalResult::ERROR;
    }
}
