...
{
    /* Patched: cfg may be NULL when called from the HAL.
     * touch_flags removed from bsp_display_cfg_t (the new struct mirrors 4b BSP
     * for HAL compatibility), so always use zero defaults here. */
    (void)cfg;
...
#if (BSP_CONFIG_NO_GRAPHIC_LIB == 0)
/*
 * Patched for Calaos Remote UI: the upstream BSP relies on esp_lv_adapter,
 * but this project uses esp_lvgl_port (same as waveshare-86-panel BSP).
 * The HAL (hal/esp32/esp32_hal_display.cpp) drives the display via
 * bsp_display_new_with_handles() + lvgl_port_* directly and never calls
 * bsp_display_start*, so the adapter-based init paths are gated out.
 * Only bsp_display_lock/unlock are exposed and routed to lvgl_port.
 */

#if 0  // Disabled: adapter-based init paths. See note above.
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg) { (void)cfg; return NULL; }
static lv_indev_t *bsp_display_indev_init(const bsp_display_cfg_t *cfg, lv_display_t *disp) { (void)cfg; (void)disp; return NULL; }
lv_display_t *bsp_display_start(void) { return NULL; }
lv_display_t *bsp_display_start_with_config(bsp_display_cfg_t *cfg) { (void)cfg; return NULL; }
#endif
