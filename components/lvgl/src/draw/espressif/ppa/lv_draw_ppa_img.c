                                 ...
                                 const lv_area_t * img_coords, const lv_area_t * clipped_img_area)
{
    ...
    /* Writeback pending SW-draw cache lines for both source and destination
     * before PPA reads them. See lv_draw_ppa_fill for rationale. */
    esp_cache_msync(dest_buf, draw_buf->data_size,
                    ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_UNALIGNED);
    esp_cache_msync((void *)src_buf, decoded->data_size,
                    ESP_CACHE_MSYNC_FLAG_DIR_C2M | ESP_CACHE_MSYNC_FLAG_UNALIGNED);
