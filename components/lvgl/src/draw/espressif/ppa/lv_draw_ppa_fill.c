                      ...
                      const lv_area_t * coords)
{
    ...
    /* Writeback any pending SW-draw cache lines to PSRAM before PPA reads
     * the buffer. Without this, the PPA fill runs against stale memory
     * contents and produces streaks where adjacent SW-drawn glyphs/labels
     * exist (LVGL 9.5 invalidate_cache_cb is not invoked between
     * interleaved SW and PPA draw tasks on the same layer buffer). */
